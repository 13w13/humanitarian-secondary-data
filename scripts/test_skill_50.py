...
DECLARED = set()  # verdicts poses a la main, sans rien executer
...
def rec_exc(tid, cat, e):
    """Une exception n'est un GAP (manque de l'outil) que si ce n'est pas une panne.

    Avant, une coupure reseau transformait chaque scenario en GAP : 29 « manques »
    qui n'en etaient pas. Panne amont ou extra optionnel absent -> SKIP.
    """
    from config import is_outage
    if is_outage(e) or isinstance(e, ImportError):
        rec(tid, cat, 'SKIP', '{}: {}'.format(type(e).__name__, str(e)[:70]))
    else:
        rec(tid, cat, 'GAP', str(e)[:80])


def declare(tid, cat, verdict, note):
    """Verdict de jugement, pas de test : compte a part pour ne pas gonfler PASS."""
    DECLARED.add(tid)
    rec(tid, cat, verdict, '[declare] ' + note)
...
def cat_A():
    ...
    try:
        ...
        rec_exc('A5', 'A', e); rec('A6', 'A', 'GAP', 'idem A5')
...
def cat_B():
    ...
    try:
        ...
        if isinstance(e, MissingCredential):
            rec('B10', 'B', 'SKIP', 'pas de cle ACLED')
        else:
            rec_exc('B10', 'B', e)
...
def cat_D():
    ...
    try:
        ...
        q(lambda: quick_chart.main([p, '--out', out]))
    ...
    try:
        sdnfile = None  # cherche un snapshot SDN telecharge dans le cache
    ...
    for tid, note in [('D1', 'download_dataset + en-tetes par hote (persona 1 verifie)'),
        ...
        declare(tid, 'D', 'PASS' if tid != 'D9' else 'GAP', note)
...
def main():
    ...
    print('\n' + '=' * 74)
    print('{} tests | PASS {} | WEAK {} | GAP {} | FALSE {} | SKIP {} | {:.0f}s'.format(
        len(R), c['PASS'], c['WEAK'], c['GAP'], c['FALSE'], c['SKIP'], time.time() - t0))
    print('dont {} verdicts DECLARES (jugement, rien n\'est execute) : {}'.format(
    ...
    with open(out, 'w', encoding='utf-8') as f:
        ...
        f.write('PASS {} · WEAK {} · GAP {} · FALSE {} · SKIP {}\n\n'.format(
            c['PASS'], c['WEAK'], c['GAP'], c['FALSE'], c['SKIP']))
        f.write('Dont {} verdicts declares (jugement, non executes).\n\n'.format(
            len(DECLARED)))
        f.write('| # | cat | verdict | note |\n|---|---|---|---|\n')
        for tid, cat, v, note in R:
            f.write('| {} | {} | {} | {} |\n'.format(tid, cat, v, note.replace('|', '/')))
    print('ecrit :', out)
    # Le runner lisait None -> "VERT" quoi qu'il arrive. Un FALSE (reponse fausse)
    # est un echec ; WEAK, GAP et SKIP restent des constats.
    return 1 if c['FALSE'] else 0
